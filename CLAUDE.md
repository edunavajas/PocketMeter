...
Bash daemon (`daemon/pocketmeter-ble-daemon.sh`) reads OAuth token, polls Anthropic API, sends JSON over BLE GATT. Run with `systemctl --user start pocketmeter-ble-daemon`. The unit file's `ExecStart` is the absolute path to the script — repoint it when switching between the worktree and the main checkout.

**Discovery & resilience:**

- Connects by name (`"Claude Controller"`) on first run, caches resolved MAC at `~/.config/pocketmeter/ble-address` and still reads the old `~/.config/claude-usage-monitor/ble-address` for compatibility. ESP32 BLE addresses are factory-burned per-chip, so swapping any board invalidates the cache.
